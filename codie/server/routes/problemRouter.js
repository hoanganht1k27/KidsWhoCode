...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        if(file.fieldname == 'problem') {
            ...
            let p = path.join(process.env.BASE_DIR, process.env.UPLOAD_TEST_DIR, req.body.problemid)
            makeDir(p)
            cb(null, p)
        }
    ...
    filename: function (req, file, cb) {
            if(file.fieldname == 'problem') {
                req.filename = req.body.problemid + '_' + file.originalname
                cb(null, req.body.problemid + '_' + file.originalname)
            } else
